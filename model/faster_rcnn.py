...
class FasterRCNN(nn.Module):
    ...
    def __init__(self, unet, rpn, head,
    ...
    def forward(self, x, scale=1.):
        ...
        * :math:`L` is the number of classes excluding the background.
        ...
        h,seg = self.extractor(x)
        rpn_locs, rpn_scores, rois, roi_indices, anchor = \
            self.rpn(h, img_size, scale)
        roi_cls_locs, roi_scores = self.head(
            h, rois, roi_indices)
        return roi_cls_locs, roi_scores, rois, roi_indices,seg# (1,1,512,512)
    ...
    def _suppress(self, raw_cls_bbox, raw_prob):
        # raw_cls_bbox:shape:(-1,self.nclass*4)
        bbox = list()
        label = list()
        score = list()
    ...
    def predict(self,imgs,sizes=None,visualize=False):
        #imgs:(1,3,H,W) masks:(1,1,H,W) imgs is no process dealed
        ...
        if visualize:
            ...
            for img in imgs:
                size = img.shape[1:]
                img= preprocess_image(at.tonumpy(img))
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            roi_cls_loc, roi_scores, rois, _ ,mask= self(img, scale=scale)
        ...
        self.use_preset('evaluate')
        self.train()
        return bboxes, labels, scores,mask #mask:(1,1,512,512)
